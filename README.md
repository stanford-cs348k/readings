...
* I'd like you to speculate a bit about the value of getting the right abstractions in place.  Consider what choices the compiler/runtime system might be able to perform for the programmer in the future given a code library written in terms of generics, interfaces, and associated types. Specifically, given information about what component classes are used with a given template, would it be valid in the future for the Slang compiler/runtime to "choose" between generating and binding highly specialized shader code (like it does today) or perhaps generate a single "general" shader that uses conditional control flow to dynamically switch between code paths depending on the types of classes that fill in the generic template?  In other words, are the abstractions set up to perform "despecialization" as an alternative implementation choice to the specialization Slang does today?  Perhaps on future GPUs it might be easier or even more performant to return to dynamic dispatch rather than worry about efficiently binding and managing thousands of specialized shaders.

* This is not a question, but a suggestion: For those wanting to go further, I recommend reading parts of [Yong He's thesis](http://graphics.cs.cmu.edu/projects/renderergenerator/yong_he_thesis.pdf) on Slang. In particular, I suspect you'll find the reflection on design principles and potential future research directions in Chapter 7 quite interesting.  
