...
__Post-Lecture Recommended Readings:__
  * If you have not taken CS248A, to prepare for lecture 2 I recommend that you watch the lecture 16 [on the digital camera pipeline](https://gfxcourses.stanford.edu/cs248a/winter26/lecture/cameraprocessing/). You can find the lecture video on the Canvas page for CS248A Winter 2026. Please ask the staff for observer access.
...
## Lecture 8: Adding Controls to Generative AI Systems ##
...
* [Unpredictable Black Boxes are Terrible Interfaces](https://magrawala.substack.com/p/unpredictable-black-boxes-are-terrible). M. Agrawala 2023
  * This is a recent blog post by Stanford Professor Maneesh Agrawala that dives into a fundamental challenge of using recent generative AI tools to create content. As part of reading the blog post, I'd like you to give the argument some context by using a modern generative AI tool like Google Gemini's Nanobanana, [Krea.ai](https://www.krea.ai/apps/image/realtime), [Midjourney's free tier](https://www.imagine.art/) or Adobe's [Firefly](https://firefly.adobe.com/), and use the tool to create an image for the following hypothetical scene: *Photo of a grassy university campus scene with two men and two women playing beach volleyball and a university tower in the background to the right, puffy white clouds in the sky. One of the women is spiking the ball down on the man's head.* (Note: that's the scene I want you to create, not necessarily the prompt I want you to use.) Before you begin, please take a moment to envision a specific version of the scene.  Take note of what objects you want, what is the composition in your head, what is the scene, etc.  You might want to describe it in detail in words.  Then get started with the AI tool.  I encourage you to post your best image on Ed for all to view!    
 ...
 * [MoVer: Motion Verification for Motion Graphics Animations](https://mover-dsl.github.io/). Ma et al. SIGGRAPH 2025
   * First some context: this paper deals with the topic of motion graphics generation. Motion graphics are 2D animations like you see [here](https://jitter.video/templates/).  There are many programming APIs for authoring motion graphics, [GSAP](https://gsap.com/showcase/) is a very popular one, and you can think of a motion graphics program as defining a timeline and specifying command about how to move objects, change colors, change shapes, etc. over time.  The design of a motion graphics API *is not* the point of this paper, the paper assumes a system like GSAP exists.  The point is to use an LLM to write motion graphics code, that *when executed* adheres to a plain the prompt.  For example, *Make the word "Kayvon" enter from the left side of the screen, bounce off the right side, and come to a stop in the center.*   
   * MoVer follows a design pattern that is being increasingly adopted across a variety of application domains: systems that LLMs to author programs that perform a desired task (in this case editing animations), and also generate auxiliary programs that *verify* that the output of the main program matches the task description. In other words, the LLM is responsible for generating a program to perform a task (in this case make an animation), and also generate the correctness tests for that program. *To help you understand the paper better, I highly recommend you start by watching the video on the paper's web site, as well as inspect the examples on that page. Click on the example images to see a detailed breakdown of what MoVer generates, and how the LLM behaves after each correction step.* Please consider the following in your response:
   * In the MoVer system, the system generates two outputs (in two different "languages") given a prompt. The first is just a python program written against a common motion graphics API. Executing this program produces a "motion graphics animation", which, in the case of this paper, is a sequence of SVG files for each animation frame. __The second is a program in the motion verification language__ that checks that certain properties hold for the motion graphics animation. *The contribution of the paper is the desgin of the motion verification language and its implementation that actually verifies animation programs Describe the primitives of the domain-specific motion verification language.* (In your answer make sure to differentiate between single-object predicates and multi-object predicates.) 
   ...
   * In the evaluation section, the paper reports in Table 3 that for 59% of the generated animations, all tests pass after the first generation attempt. And in 93.6\% of cases, all verification tests pass after multiple animation generation tries (the result in table 2).  What is the stated reason for the increased success with multiple tries? How do we know MoVer is providing a benefit against a simple baseline? (hint: what is that simple baseline?)
   * The paper states that the results in Table 3 were computed by using "ground truth" MoVer programs, not the MoVer programs generated by the system. This is a subtle difference. Why do you think the authors did this? Is it good scientific evaluation?
...
* [Genie: Generative Interactive Environments](https://sites.google.com/view/genie-2024/home). Bruce et al. 2024
   ...
   * Despite the task desition above, the input to the training procedure is just unlabeled video.  Please describe what is meant by the "latent action" that the method attempts to infer from this unlabeled video.  
   * Please describe the inputs and outputs of each step of the process (the video tokenzier, the latent action model, and the dynamics model).  What is the responsibility of each step? The details of how the steps are implemented with a spatiotemporal transformer architecture are not relevant to our in-class discussion. Students without ML background can skip the details of the architectural implementation -- this details are deprecated anyway since modern implementations like Genie 3 are likely based on diffusion -- but I want everyone to understand how the pieces fit together.
   ...
   * If you wanted to train the most general and capable AI agents the world has ever seen... do you feel it is more promising to push forward on the approach of learning interactive world simulators from large-scale observations, or whether it's more promising to invest in technology of faster, more detailed world simulation engines like a really high-end game engine. __You have to take a side!__  Please be prepared to argue with your classmates in favor of one side of the debate.  Here are some issues to consider:
      *  What are the pros/cons of each approach? Consider:
          ...
          * Consider procedural approaches to creating content, like [ProcThor](https://procthor.allenai.org/), [InfiniGen](https://infinigen.org/), or [XLand](https://deepmind.google/discover/blog/generally-capable-agents-emerge-from-open-ended-play/) or [Kinetix](https://kinetix-env.github.io/).
...
Please see the lecture 12 post-lecture reading.

## Lecture 14: Using Agents to Understand and Analyze Simulated Worlds # 
